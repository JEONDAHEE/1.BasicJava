...
//주석: 프로그램 코드로 인식하지 않는 문장(주로 코드를 설명할떄 사용한다.)
//한줄주석 : ctrl + shift + c
/* 범위주석 : ctrl + shift + / (해제:\)
 * 
 */
...
//프로그램이란? 어떠한 데이터를 목적에따라 처리하는 것이다.
//프로그램은 데이터와 처리로 구성되어있다.

//자바프로그램은 하나 이상의 클래스로 구성되어 있고 클래스 안에 데이터와 처리내용을 작성한다.
...
public class ABC {
	...
	public static void main(String[] args) { //Java에서는 중괄호를 선언부 옆에 넣는다.
		
		//블럭이 시작되면 tap(탭) 1칸으로 계층을 표현(계층을 쉽게 구분하기 위해)
		...
		//코드는 영문으로 사용하기 때문에 영타연습이 필요하다. 최소한 200타 이상 나오도록 연습하자! 다히 사랑해 !
