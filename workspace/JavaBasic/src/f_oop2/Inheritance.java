...
public class Inheritance {
	...
	public static void main(String[] args) {
		/*
		 ...
		 - 부모클래스의 생성자와 초기화 블럭을 제외한 모든 멤버를 물려 받는다.
		 - 하나의 클래스만 상속받을 수 있다.
		 - 상속받지 않는 모든 클래스는 object클래스를 상속받는다.
		 - 자식클래스는 부모클래스의 멤버 외의 새로운 멤버를 가질 수 있으므로 자식클래스는 부모 클래스보다 크거나 같다.
		 - 두개이상의 클래스를 만드는 데 공통된 멤버가 있는 경우 부모클래스로 만든다.
		 
		 <<super>>
		 - 부모클래스의 인스턴스 주소가 저장된 변수이다.
		 - 부모클래스의 멤버와 자식 클래스의  멤버의 이름이 중복 될 때 둘을 구분하기 위해 사용한다.
		 
		 <<super()>>
		 - 생성자에서 부모클래스의 생성자를 호출하기 위해 사용한다.
		 - 자식 클래스 생성자의 첫 줄에 super()가 있어야한다.
		 - 없으면 컴파일러가 자동으로 추가한다.
		 
		 <<오버라이딩>>
		 - 상속받은 메서드의 내용을 재정의하는것이다.
...
//		t.setMinute(-9);
		...
		System.out.println(new Inheritance());//두개가 서로 다른 주소를 가지고 있다.
		
		System.out.println(Singleton.getInstance());
		System.out.println(Singleton.getInstance());//두개가 같은 주소를 가지고 있다. = 같은객체다.
