...
public class EXAM_05 {
	...
	public static void main(String[] args) {
		      ...
		      /*
		       ...
		       b. int[] arr = {1,2,3,};
		       c. int[] arr = new int[5];
		       d. int[] arr = new int[5]{1,2,3,4,5};//초기화할땐 크지를 지정해주거나 공간에 값을 넣어주거나 한가지만 해야 한다. 둘다해서 문제.
		       e. int arr[5];//선언힐때 크기를 지정 할 수 없음
		       f. int[] arr[] = new int[3][];
		      ...
		      for(int i = 0; i < arr.length; i++){
		            ...
		            average = total / (float)(arr.length * arr[0].length);//행과 열의 길이를 나눠줘야 하기 떄문에.
		         }
		      ---------------------------------------------------------------------------------------------
		      ...
		      for(int i = 0; i < star.length; i++) {
		         for(int j = 0; j < star[i].length; j++) {
		            System.out.println();
		              int a = j; // 원래위치 : (i,j), 옮긴위치 : (a, b) 라고 할때, 옮긴위치 a는 원래위치 j와 똑같음
		              int b = (star.length-1)-i; 
		              // i와 b는 합이 항상 3 이기 때문에 i + b = star.length(= 4) - 1
		              // 즉, i = (star.length-1)-i 가 된다.  
		              
		              result[a][b] = star[i][j]; // (i, j)를 (a, b)로 바꿔준다.
		          }
		      }
		      for(int i = 0; i < result.length; i++) {
		         for(int j = 0; j < result[i].length; j++) {
		            System.out.print(result[i][j]);//[5][4]를 출력해준다.
		         }
		         System.out.println();
		      }
		      ---------------------------------------------------------------------------------------------
		      ...
		      ---------------------------------------------------------------------------------------------
		      [5-11] 주어진 2차원 배열의 데이터보다 가로와 세로로 1이 더 큰 배열을 생성해서 배열의 행과 열의 마지막 요소에 
